...
import com.atsuishio.superbwarfare.client.model.entity.F16aModel;
...
import static com.atsuishio.superbwarfare.entity.vehicle.F16aEntity.LOADED_BOMB;
import static com.atsuishio.superbwarfare.entity.vehicle.F16aEntity.LOADED_MISSILE;
import static com.atsuishio.superbwarfare.entity.vehicle.F16aEntity.LOADED_AAM;
import static com.atsuishio.superbwarfare.entity.vehicle.base.MobileVehicleEntity.GEAR_ROT;

public class F16aRenderer extends GeoEntityRenderer<F16aEntity> {

    public F16aRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new F16aModel());
        this.shadowRadius = 0.5f;
    }

    @Override
    public RenderType getRenderType(F16aEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, F16aEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(F16aEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, F16aEntity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (name.equals("bomb1")) {
            bone.setHidden(animatable.getEntityData().get(LOADED_BOMB) < 4);
        }
        ...
        if (name.equals("irhaam1")) {
            bone.setHidden(animatable.getEntityData().get(LOADED_AAM) < 4);
        }
        if (name.equals("irhaam2")) {
            bone.setHidden(animatable.getEntityData().get(LOADED_AAM) < 3);
        }
        if (name.equals("irhaam3")) {
            bone.setHidden(animatable.getEntityData().get(LOADED_AAM) < 2);
        }
        if (name.equals("irhaam4")) {
            bone.setHidden(animatable.getEntityData().get(LOADED_AAM) < 1);
        }
