...
public class ArtilleryEntity extends VehicleEntity implements GeoEntity {

    public static final EntityDataAccessor<Integer> FIRE_TIME = SynchedEntityData.defineId(ArtilleryEntity.class, EntityDataSerializers.INT);
    public static final EntityDataAccessor<Float> PITCH = SynchedEntityData.defineId(ArtilleryEntity.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> YAW = SynchedEntityData.defineId(ArtilleryEntity.class, EntityDataSerializers.FLOAT);

    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public ArtilleryEntity(PlayMessages.SpawnEntity packet, Level level) {
        this(ModEntities.ARTILLERY.get(), level);
    }

    public ArtilleryEntity(EntityType<ArtilleryEntity> type, Level level) {
        super(type, level);
    }

    public ArtilleryEntity(Level level, float yRot) {
        super(ModEntities.ARTILLERY.get(), level);
        this.setYRot(yRot);
    ...
    public @NotNull InteractionResult interact(Player player, @NotNull InteractionHand hand) {
        ...
        if (stack.getItem() instanceof MortarShell shell && !player.isShiftKeyDown() && this.entityData.get(FIRE_TIME) == 0) {
            this.entityData.set(FIRE_TIME, 200);
            ...
            Mod.queueServerWork(200, () -> {
                Level level = this.level();
                if (level instanceof ServerLevel server) {
                    float yawRad = (float) Math.toRadians(this.getYRot());
                    double offsetX = -Mth.sin(yawRad) * 0;
                    double offsetZ = Mth.cos(yawRad) * 11;
                    double offsetY = 5; // 少し高めの砲口
                    MortarShellEntity entityToSpawn = shell.createShell(player, level, stack);
                    entityToSpawn.setPos(this.getX() + offsetX, this.getY() + offsetY, this.getZ() + offsetZ);
        ...
        if (player.getMainHandItem().getItem() == ModItems.FIRING_PARAMETERS.get()) {
            if (setTarget(player.getMainHandItem())) {
                ...
                player.displayClientMessage(Component.translatable("tips.superbwarfare.artillery.warn").withStyle(ChatFormatting.RED), true);
    ...
    public void travel() {
        ...
        this.setXRot(Mth.clamp(this.getXRot() + Mth.clamp(0.5f * diffX, -20f, 20f), -30, -10));
    }

    private PlayState movementPredicate(AnimationState<ArtilleryEntity> event) {
        if (this.entityData.get(FIRE_TIME) > 0) {
            return event.setAndContinue(RawAnimation.begin().thenLoop("animation.artillery.fire"));
        }
        return event.setAndContinue(RawAnimation.begin().thenLoop("animation.artillery.idle"));
    }
