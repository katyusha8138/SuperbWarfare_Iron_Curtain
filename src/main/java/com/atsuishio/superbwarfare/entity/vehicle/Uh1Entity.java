...
public class Uh1Entity extends ContainerMobileVehicleEntity implements GeoEntity, HelicopterEntity, WeaponVehicleEntity {

    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    public static final EntityDataAccessor<Float> PROPELLER_ROT = SynchedEntityData.defineId(Uh1Entity.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Integer> LOADED_ROCKET = SynchedEntityData.defineId(Uh1Entity.class, EntityDataSerializers.INT);
    ...
    public Uh1Entity(PlayMessages.SpawnEntity packet, Level world) {
        this(ModEntities.UH_1.get(), world);
        ...
        return new VehicleWeapon[][]{
                new VehicleWeapon[]{
                        new ProjectileWeapon()
                                .damage(VehicleConfig.UH_1_GUN_DAMAGE.get())
                                .sound(ModSounds.INTO_CANNON.get())
                                .icon(Mod.loc("textures/screens/vehicle_weapon/gun_7_62mm.png"))
                                .sound1p(ModSounds.MINIGUN_FIRE_1P.get())
                                .sound3p(ModSounds.MINIGUN_FIRE_3P.get())
                                .sound3pFar(ModSounds.MINIGUN_FAR.get())
                                .sound3pVeryFar(ModSounds.MINIGUN_VERYFAR.get()),
                        new HeliRocketWeapon()
                                .damage(VehicleConfig.UH_1_ROCKET_DAMAGE.get())
                                .explosionDamage(VehicleConfig.UH_1_ROCKET_EXPLOSION_DAMAGE.get())
                                .explosionRadius(VehicleConfig.UH_1_ROCKET_EXPLOSION_RADIUS.get())
    ...
    public @NotNull InteractionResult interact(Player player, @NotNull InteractionHand hand) {
        ...
        if (stack.getItem() == ModItems.ROCKET_70.get() && this.entityData.get(LOADED_ROCKET) < 7) {
            // 装载火箭
    ...
    private void handleAmmo() {
        ...
        if ((hasItem(ModItems.ROCKET_70.get()) || InventoryTool.hasCreativeAmmoBox(player)) && reloadCoolDown == 0 && this.getEntityData().get(LOADED_ROCKET) < 7) {
    ...
    public void travel() {
        ...
        if (engineStart) {
            this.consumeEnergy((int) (VehicleConfig.UH_1_MIN_ENERGY_COST.get() + this.entityData.get(POWER) * ((VehicleConfig.UH_1_MAX_ENERGY_COST.get() - VehicleConfig.UH_1_MIN_ENERGY_COST.get()) / 0.12)));
        }
    ...
    public void positionRider(@NotNull Entity passenger, @NotNull MoveFunction callback) {
        ...
        if (i == 0) {
            Vector4f worldPosition = transformPosition(transform, x, y, z + 2.7f);
            ...
            Vector4f worldPosition = transformPosition(transform, -0.7f, -1, 0);
            passenger.setPos(worldPosition.x, worldPosition.y, worldPosition.z);
            callback.accept(passenger, worldPosition.x, worldPosition.y, worldPosition.z);
        } else if (i == 3) {
            Vector4f worldPosition = transformPosition(transform, 0.7f, -1, 0);
    ...
    public float getMaxHealth() {
        return VehicleConfig.UH_1_HP.get();
    }

    @Override
    public int getMaxEnergy() {
        return VehicleConfig.UH_1_MAX_ENERGY.get();
    }
    ...
    public void vehicleShoot(Player player, int type) {
        ...
        if (getWeaponIndex(0) == 0) {
            ...
            if (this.entityData.get(AMMO) > 0 || hasCreativeAmmo) {
                var entityToSpawn = ((ProjectileWeapon) getWeapon(0)).create(player);
