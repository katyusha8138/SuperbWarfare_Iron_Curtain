...
public class Uh60Entity extends ContainerMobileVehicleEntity implements GeoEntity, HelicopterEntity, WeaponVehicleEntity {
    ...
    public Uh60Entity(PlayMessages.SpawnEntity packet, Level world) {
        this(ModEntities.UH_60.get(), world);
        ...
        return new VehicleWeapon[][]{
                new VehicleWeapon[]{
                        new ProjectileWeapon()
                                .damage(VehicleConfig.UH_60_GUN_DAMAGE.get())
    ...
    public void travel() {
        ...
        if (engineStart) {
            this.consumeEnergy((int) (VehicleConfig.UH_60_MIN_ENERGY_COST.get() + this.entityData.get(POWER) * ((VehicleConfig.UH_60_MAX_ENERGY_COST.get() - VehicleConfig.UH_60_MIN_ENERGY_COST.get()) / 0.12)));
        }
    ...
    protected void clampRotation(Entity entity) {
        if (entity == getNthEntity(0) || entity == getNthEntity(1)) {
            ...
            float f3 = Mth.clamp(f2, -80.0F, 80F);
            entity.yRotO += f3 - f2;
            entity.setYRot(entity.getYRot() + f3 - f2);
            entity.setYBodyRot(this.getYRot());
        } else if(entity == getNthEntity(2) || entity == getNthEntity(3)) {
            float f = Mth.wrapDegrees(entity.getXRot());
            float f1 = Mth.clamp(f, -80.0F, 80F);
            entity.xRotO += f1 - f;
            entity.setXRot(entity.getXRot() + f1 - f);
    ...
    public void positionRider(@NotNull Entity passenger, @NotNull MoveFunction callback) {
        ...
        if (i == 0) {
            Vector4f worldPosition = transformPosition(transform, x, y, z + 4f);
            ...
            Vector4f worldPosition = transformPosition(transform, -0.7f, y, z + 1.2f);
            passenger.setPos(worldPosition.x, worldPosition.y, worldPosition.z);
            callback.accept(passenger, worldPosition.x, worldPosition.y, worldPosition.z);
        } else if (i == 3) {
            Vector4f worldPosition = transformPosition(transform, 0.7f, y, z + 2f);
            passenger.setPos(worldPosition.x, worldPosition.y, worldPosition.z);
            callback.accept(passenger, worldPosition.x, worldPosition.y, worldPosition.z);
        } else if (i == 4) {
            Vector4f worldPosition = transformPosition(transform, 0.7f, y, 0);
            passenger.setPos(worldPosition.x, worldPosition.y, worldPosition.z);
            callback.accept(passenger, worldPosition.x, worldPosition.y, worldPosition.z);
        } else if (i == 5) {
            Vector4f worldPosition = transformPosition(transform, 0.7f, y, 0);
    ...
    public float getMaxHealth() {
        return VehicleConfig.UH_60_HP.get();
    }

    @Override
    public int getMaxEnergy() {
        return VehicleConfig.UH_60_MAX_ENERGY.get();
    }
